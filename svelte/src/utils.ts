...
interface Compression {
  content: sjcl.BitArray
  compression: sjcl.BitArray
}

export function compress(obj: string): Compression {
  const uint8array = new TextEncoder().encode(obj)
...
export function decompress(obj: sjcl.BitArray): string {
...
export function BitsToUint8Array(bits: sjcl.BitArray): Uint8Array {
  const array = [], hex = sjcl.codec.hex.fromBits(bits)
